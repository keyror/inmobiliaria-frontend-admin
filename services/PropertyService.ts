...
class PropertyService {
    ...
    async createProperty(payload: any): Promise<any> {
        return useApi(ApiUrls.PROPERTIES_CREATE_POST, {method: "POST", body: payload});
    }

    async updateProperty(id: string | number, payload: Partial<IProperty>): Promise<any> {
        return useApi(`${ApiUrls.PROPERTIES_UPDATE_PUT}/${id}`, {method: "PUT", body: payload});
    }

    async deleteProperty(id: string | number): Promise<any> {
        return useApi(`${ApiUrls.PROPERTIES_DELETE_DELETE}/${id}`, {method: "DELETE"});
    }
}
