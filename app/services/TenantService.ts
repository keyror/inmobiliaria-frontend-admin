...
class TenantService {
  ...
  async createTenant(payload: any): Promise<any> {
    return useApi(ApiUrls.TENANTS_CREATE_POST, {
      method: "POST",
      body: payload,
    });
  }
  ...
  async updateTenant(id: number | string, payload: any): Promise<any> {
    return useApi(`${ApiUrls.TENANTS_UPDATE_PUT}/${id}`, {
      method: "PUT",
      body: payload,
    });
  }
  ...
  async deleteTenant(id: number | string): Promise<any> {
    return useApi(`${ApiUrls.TENANTS_DELETE_DELETE}/${id}`, {
      method: "DELETE",
    });
  }

  async activateTenant(id: number | string): Promise<any> {
    return useApi(`${ApiUrls.TENANTS_ACTIVATE_PATCH}/${id}/activate`, {
      method: "PATCH",
    });
  }

  async deactivateTenant(id: number | string): Promise<any> {
    return useApi(`${ApiUrls.TENANTS_DEACTIVATE_PATCH}/${id}/deactivate`, {
      method: "PATCH",
    });
  }
}
