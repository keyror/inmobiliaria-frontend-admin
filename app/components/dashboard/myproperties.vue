        ...
        <div class="media" v-for="(item, index) in alldata" :key="index">
          ...
          <div class="media-body">
            ...
            <div>
              ...
              <span class="label label-light" :class="item.statuscolor">{{
                item.status
              }}</span>
            </div>
          </div>
...
interface DashboardResponse {
  my_property: PropertyItem[];
}
const alldata = ref<PropertyItem[]>([]);
const { data } = await useFetch<DashboardResponse>(
  baseUrl + "/data/dashboard.json",
);
watchEffect(() => {
  if (data.value) {
