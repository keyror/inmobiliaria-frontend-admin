...
const buttonText = computed(() => {
  ...
  if (!hasFilter) {
    return allSelected.value ? "Deseleccionar todos" : "Seleccionar todos";
  } else {
    if (allFilteredSelected.value && !hasSelectedOutsideFilter.value) {
...
const getSelectedCount = (module) => {
  ...
  return modulePerms.filter((p) => props.selectedPermissions.includes(p.id))
    .length;
};
</script>
              ...
              >
                ...
                >
                  <div class="d-flex align-items-center">
                    ...
                    <span class="text-capitalize fw-semibold module-name">{{
                      module
                    }}</span>
                    <span
                      class="badge ms-2"
