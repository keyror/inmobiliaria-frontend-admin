      ...
      <div class="card-header admin-form pb-0">
        <div class="row g-2 align-items-center">
          ...
          <div class="col-12 col-md-4 col-lg-6">
            <div
              class="d-flex gap-2 justify-content-start justify-content-md-end flex-wrap"
            >
              <!-- Botón Actualizar -->
              <button
            ...
            >
              ...
              >
                <ul class="pagination mb-0 flex-wrap">
                  ...
                  <li
                    :class="{ disabled: serverOptions.page === totalPages }"
                    class="page-item"
                  >
                    <a
                      aria-label="Next"
...
import type {
  IExportOption,
  IExportOptions,
} from "@/interfaces/IExportOptions";
...
const totalPages = computed(() =>
  Math.max(
    1,
    Math.ceil(props.serverItemsLength / serverOptions.value.rowsPerPage),
  ),
);
...
const paginationText = computed(() => {
  ...
  const end = Math.min(
    serverOptions.value.page * serverOptions.value.rowsPerPage,
...
const exportFile = async (exportOption: IExportOption) => {
  ...
  SaveFileFromBackService.getExport(paramsTable.value, exportOption.url)
    .then((response) => {
      SaveFileFromBackService.captureFile(
        response,
        exportOption.extension,
        props.exportInput?.name ?? "archivo",
      );
      LoadingService.hide();
      AlertaService.showSuccess(
        "Operación Exitosa",
